...
public HashMap<String,String> retrieve () throws ClassNotFoundException, SQLException {
        ...
        //ResultSet rs = stmt.executeQuery("Select userid,access_token from algotrade.user_token_db where platform='ALICE' and is_sigma = 1  and is_active = 1 and is_day_trading_active = 1 and timestamp > '"+LocalDateTime.now().getYear()+"-"+LocalDateTime.now().getMonthValue()+"-"+LocalDateTime.now().getDayOfMonth()+" 08:00:00';");
