...
public class OptionSigmaBNFPE_Sell implements Runnable{
	...
	public void run() {
		...
		System.out.println(LocalDateTime.now().format(formatter)+" : "+instrumentPEPrimary+" "+numberFormat.format(closePrice.getValue(seriesBarCount-1).doubleValue())+"|"+numberFormat.format(vwap.get(seriesBarCount-1).doubleValue())
		...
		while(startPEModule) {
			...
			if(currentTime.getSecond()==0) {
				...
				if(series.getBar(seriesBarCount-1).getClosePrice().isLessThan(superTrend.get(seriesBarCount-1)) && newSeries
					...
					&& stoch_K.getValue(seriesBarCount-1).isLessThan(stoch_D.getValue(seriesBarCount-1)) //&& stoch_K.getValue(seriesBarCount-1).isLessThan(80)
					&& rsi_K.getValue(seriesBarCount-1).isLessThan(rsi_D.getValue(seriesBarCount-1)) //&& rsi_K.getValue(seriesBarCount-1).isLessThan(80)
